...
class AliyunService: ObservableObject {
    ...
    func generateAICollage(imageDatas: [Data], itemDescriptions: [String]) async throws -> Data {
        ...
        let itemList = itemDescriptions.enumerated()
            ...
            .joined(separator: "、")
        let prompt = """
        上方共 \(n) 张服装图，编号分别为：\(itemList)。\
        这 \(n) 件是完全不同的独立单品，每件都必须出现在画面中。\
        硬规则（不可违反，违反即视为生成失败）：\
        ①画面服装件数恰好为 \(n) 件，不多不少；\
        ②每件服装在画面中只能出现一次，严禁将任何一件重复绘制两次或更多次；\
        ③仅使用以上 \(n) 张图片中的服装，不得凭空添加未出现在输入图片中的任何单品；\
        ④每件保持原图颜色、图案、款式、长度完全不变，不得修改任何细节。\
        视觉软要求（尽量满足）：\
        ·纯白背景，整洁干净；\
        ·俯视平铺 flat lay，上装居上、下装居中、鞋包居下；\
        ·衣物之间自然错落、轻微叠压，局部微微折叠产生自然褶皱感；\
        ·每件衣物可略微旋转 5-15 度，增加构图动感，避免呆板的平行排列；\
        ·柔和漫射光源，衣物边缘有轻微投影，增强立体层次感；\
        ·整体构图留有适当留白，风格参考高端时尚杂志 flat lay 大片。
        """
